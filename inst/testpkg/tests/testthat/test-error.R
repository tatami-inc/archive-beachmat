...
test_that("Sparse matrix errors thrown", {
    ...
    wrong@p[1] <- -1L
    expect_fixed_error(.Call(beachtest:::cxx_test_numeric_access, wrong, 1L), "first element of 'p' in a dgCMatrix object should be 0")
    wrong <- A
    wrong@p[ncol(A)+1] <- -1L
    expect_fixed_error(.Call(beachtest:::cxx_test_numeric_access, wrong, 1L), "last element of 'p' in a dgCMatrix object should be 'length(x)'")
    wrong <- A
    wrong@p[2] <- wrong@p[2]+100L
    expect_fixed_error(.Call(beachtest:::cxx_test_numeric_access, wrong, 1L), "'p' slot in a dgCMatrix object should be sorted")
    wrong <- A
    wrong@p <- wrong@p[1]
    expect_fixed_error(.Call(beachtest:::cxx_test_numeric_access, wrong, 1L), "length of 'p' slot in a dgCMatrix object should be equal to 'ncol+1'")
    ...
    wrong@i <- rev(wrong@i)
    expect_fixed_error(.Call(beachtest:::cxx_test_numeric_access, wrong, 1L),  "'i' in each column of a dgCMatrix object should be sorted")
    wrong <- A
    wrong@i <- wrong@i[1]
    expect_fixed_error(.Call(beachtest:::cxx_test_numeric_access, wrong, 1L),  "'x' and 'i' slots in a dgCMatrix object should have the same length")
    wrong <- A
    wrong@i <- wrong@i*100L
    expect_fixed_error(.Call(beachtest:::cxx_test_numeric_access, wrong, 1L),  "'i' slot in a dgCMatrix object should contain elements in [0, nrow)")
    ...
    expect_fixed_error(.Call(beachtest:::cxx_test_numeric_access, A, 1L), "dgTMatrix not supported, convert to dgCMatrix")
    
    B <- A!=0
    expect_fixed_error(.Call(beachtest:::cxx_test_logical_access, B, 1L), "lgTMatrix not supported, convert to lgCMatrix")
})
...
test_that("Sparse matrix errors thrown", {
    ...
    wrong <- A
    wrong@uplo <- "W"
    expect_fixed_error(.Call(beachtest:::cxx_test_numeric_access, wrong, 1L), "'uplo' slot in a dspMatrix object should be either 'U' or 'L'")
...
test_that("HDF5 matrix errors thrown", {
    wrong <- A
    wrong@seed <- Matrix(1)
    expect_fixed_error(.Call(beachtest:::cxx_test_numeric_access, wrong, 1L), "'seed' slot in a HDF5Matrix object should be a HDF5ArraySeed object")
    ...
    expect_fixed_error(.Call(beachtest:::cxx_test_numeric_access, wrong, 1L), "'file' slot in a HDF5ArraySeed object should be a string")
...
test_that("Logical errors thrown", {
    b <- matrix(100L, 10, 10)
    expect_fixed_error(.Call(beachtest:::cxx_test_logical_access, b, 1L), "matrix should be logical")
...
test_that("Integer errors thrown", {
    ...
    test.mat <- matrix(150, 15, 10)
    B <- as(test.mat, "HDF5Array")
    expect_fixed_error(.Call(beachtest:::cxx_test_integer_access, B, 1L), "'first_val' slot in a HDF5ArraySeed object should be integer")
