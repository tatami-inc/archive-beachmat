...
```
BEACH_LIBS=$(shell echo 'beachmat::pkgconfig("PKG_LIBS")'|\
...
```
BEACH_LIBS=`echo 'beachmat::pkgconfig("PKG_LIBS")'|\
    "${R_HOME}/bin/R" --vanilla --slave`
PKG_LIBS=$(BEACH_LIBS)
```
...
This will be a separate file from what would normally be used by the `r Biocpkg("HDF5Array")` methods via `getHDF5DumpFile()`.
...
- The API is not thread-safe, due to the use of cached class members and the potential for writing to the same location on disk/memory.
The former can be solved by using `clone` to create `*_output` copies for use in each thread.
