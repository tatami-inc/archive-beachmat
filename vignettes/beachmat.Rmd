...
- `dptr->get_col(c, X)` takes a pointer `X` to an array of doubles of length `nrow`, and fills it with values at column `c`.
It returns nothing.
- `dptr->get_row(r, X)` takes a pointer `X` to an array of doubles of length `ncol`, and fills it with values at row `r`.
It returns nothing.
...
For integer and logical matrices, `X` should be an integer pointer to an array of the specified length, and `get` will return an integer.
For character matrices, `X` should point to an array of `const char*`, and `get` will return a `const char*`.

## Important developer information 

- R stores dimensions as integers rather than as `size_t`, so we simply follow suit when calling `get_nrow` and `get_ncol`.
- In theory, greater efficiency could be obtained for `get_col` on base matrices by returning a pointer to the column, rather than copying the data.
...
This would be dangerous as the internal storage will change upon repeated calls to `get_col`.
Filling a user-supplied array is clearer as it is obvious that the array will change when `get_col` is called again.
...
- When using character matrices, each `const char*` value points to a null-terminated C-style string stored in R's global cache.
These can be assumed to persist for the lifetime of the `character_matrix` object, but should _not_ be modified.
...
-- Storage of CHARSXPs in a StringVector for HDF5_character_matrices is the simplest solution to ensure they don't get garbage collected.
-->
