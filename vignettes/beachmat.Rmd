...
- `r CRANpkg("Rcpp")` and `r Biocpkg("Rhdf5lib")` should be installed.
...
- `dptr->get_col(c, X)` takes a `Rcpp::Vector::iterator` object`X` and fills it with values at column `c`.
...
- `dptr->get_col(c, X, rstart, rend)` takes a `Rcpp::Vector::iterator` object `X` and fills it with values at column `c` from row `rstart` to `rend-1`.
There should be at least `rend-rstart` accessible elements, i.e., `*X` and `*(X+rend-rstart-1)` should be valid entries.
No value is returned by this method.
- `dptr->get_row(r, X)` takes an `Rcpp::Vetor::iterator` object `X` and fills it with values at row `r`.
There should be at least `ncol` accessible elements, i.e., `*X` and `*(X+ncol-1)` should be valid entries.
No value is returned by this method.
- `dptr->get_row(r, X, cstart, cend)` takes an `Rcpp::Vetor::iterator` object `X` and fills it with values at row `r` from column `cstart` to `cend-1`.
...
- `dptr->get_matrix_type()` returns a `matrix_type` value specifying the type of matrix pointed to by `dptr`.
...
Furthermore, `rend >= rstart` and `cend >= cstart` should be true.
...
There are additional methods that provide some advantages for specific matrix types:

- `dptr->get_const_col(c, work, rstart, rend)` returns a `Rcpp::Vector::const_iterator` pointing to `rstart` row of column `c`.
...
- `dptr->get_nonzero_col(c, index, values, rstart, rend)` takes a `Rcpp::IntegerVector::iterator` object `index` and a `Rcpp::Vector::iterator` object `values`.
For each non-zero entry in column `c` from rows `[rstart, rend)`, its row index is stored in the memory pointed to by `index` and its value is stored in `values`.
(Both iterators should point to memory with at least `rend-rstart` addressable elements.)
The return value of the function isThe number of non-zero entries stored in this manner.
This is quite efficient for sparse matrices; for all other matrices, `get_col` is called and zeros are stripped out afterwards.
- `dptr->get_nonzero_col(r, index, values, cstart, cend)` takes a `Rcpp::IntegerVector::iterator` object `index` and a `Rcpp::Vector::iterator` object `values`.
For each non-zero entry in row `r` from columns `[cstart, cend)`, its column index is stored in the memory pointed to by `index` and its value is stored in `values`.
(Both iterators should point to memory with at least `cend-cstart` addressable elements.)
The return value of the function isThe number of non-zero entries stored in this manner.
This is quite efficient for sparse matrices; for all other matrices, `get_row` is called and zeros are stripped out afterwards.

The `get_nonzero_*` functions are obviously not available for character matrices.
...
```
std::unique_ptr<numeric_output> odmat=beachmat::create_numeric_output(nrow, ncol, beachmat::SIMPLE);
```

A sparse matrix is similarly created by setting the last argument to `beachmat::SPARSE`, while a `HDF5Matrix` is constructed by setting `beachmat::HDF5`.
...
Dynamic choice of output type is performed by supplying the `Rcpp::Robject` object containing the input matrix.
If I have a matrix object `dmat`, the output type can be matched to the input type with:

```
std::unique_ptr<numeric_output> odmat=beachmat::create_numeric_output(nrow, ncol, dmat, 
    /* simplify = */ true, /* preserve_zero = */ false);
```

The second-last argument `simplify` indicates whether non-`matrix` input objects should be "simplified" to a `matrix` output object.
If `false`, a `HDF5Matrix` output object will be returned instead.

The last argument `preserve_zero` indicates whether a `*gCMatrix` input should result in a `dgCMatrix` output.
Exact zeroes are detected and ignored when filling this matrix.
Also note that if `preserve_zero` is `true`, the setting of `simplify` is ignored when a sparse input matrix is supplied.
