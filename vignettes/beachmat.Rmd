...
- `dptr->get_row(r)` returns a constant pointer to an array of doubles of length equal to `ncol`, containing values at row `r`.
- `dptr->get_col(c)` returns a constant pointer to an array of doubles of length equal to `nrow`, containing values at column `c`.
...
As a general rule, if it can be stored in a `SummarizedExperiment` class (from the `r Biocpkg("SummarizedExperiment")` package), the API should be able to handle it.
Please contact the maintainers if you have a class that you would like to be supported.
