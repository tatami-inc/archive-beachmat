...
_beachmat_ itself should be added to the `Suggests` field, which prevents esoteric errors under certain `R CMD check` settings.
If you intend to accept instances of `r CRANpkg("Matrix")`, `r Biocpkg("HDF5Array")` or `r Biocpkg("DelayedArray")` classes, these packages should also be listed in the `Suggests` field, if they are not already in `Imports` or `Depends`:

```
Suggests: beachmat, Matrix, HDF5Array, DelayedArray
```
